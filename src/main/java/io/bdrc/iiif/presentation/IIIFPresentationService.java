...
public class IIIFPresentationService {
    ...
    public ResponseEntity<Object> getKey(@PathVariable String region, @PathVariable final String key)
        ...
        // this is quite repetitive unfortunately but I couldn't find another
        // way...
        switch (prefix) {
            ...
            default :
                throw new BDRCAPIException(500, AppConstants.GENERIC_IDENTIFIER_ERROR,
                        "unhandled key prefix, this shouldn't happen");
        }
    }

    @RequestMapping(value = "/collection/{identifier}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<StreamingResponseBody> getCollectionNoVer(@PathVariable String identifier,
    ...
    public ResponseEntity<StreamingResponseBody> getCollection(@PathVariable String identifier,
            @PathVariable String version, HttpServletRequest request, HttpServletResponse resp)
            throws BDRCAPIException {
        ...
        if (access == AccessType.OPEN) {
            return ResponseEntity.ok().cacheControl(CacheControl.maxAge(maxAgeSeconds, TimeUnit.SECONDS).cachePublic())
                    .body(getStream(coll));
        } else {
            return ResponseEntity.ok().cacheControl(CacheControl.maxAge(maxAgeSeconds, TimeUnit.SECONDS).cachePrivate())
                    .body(getStream(coll));
        }
    }

    @RequestMapping(value = "/{identifier}/manifest", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<StreamingResponseBody> getManifestNoVer(@PathVariable String identifier,
    ...
    public ResponseEntity<StreamingResponseBody> getManifest(@PathVariable String identifier,
            @PathVariable String version, HttpServletRequest req, HttpServletResponse resp) throws BDRCAPIException {
        resp.setContentType("application/json;charset=UTF-8");
        ...
        if (id.getInstanceId() != null && (id.getSubType() == Identifier.MANIFEST_ID_WORK_IN_ITEM
        ...
        // quite specific: we want to compute wo here only if we don't have the
        // volume
        // id
        logger.debug("for {} got ID {}", identifier, id);
        if (id.getSubType() == Identifier.MANIFEST_ID_WORK_IN_VOLUMEID_OUTLINE && id.getInstanceId() != null
                && id.getImageGroupId() == null) {
            try {
        ...
        if (volumeId == null) {
            ...
            if (volumeId == null) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).cacheControl(CacheControl.noCache())
                        .body(getStream("\"Cannot find volume ID\""));
            }
        }
        ImageGroupInfo vi;
        try {
        ...
        if (vi.access == AccessType.OPEN) {
            return ResponseEntity.status(HttpStatus.OK)
                    .cacheControl(CacheControl
                            .maxAge(Long.parseLong(AuthProps.getProperty("max-age")), TimeUnit.SECONDS).cachePublic())
                    .body(getStream(resmanifest));
        } else {
            return ResponseEntity.status(HttpStatus.OK)
                    .cacheControl(CacheControl
                            .maxAge(Long.parseLong(AuthProps.getProperty("max-age")), TimeUnit.SECONDS).cachePrivate())
                    .body(getStream(resmanifest));
        }
    }

    @RequestMapping(value = "/{identifier}/canvas/{filename}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<StreamingResponseBody> getCanvasNoVer(@PathVariable String identifier,
    ...
    public ResponseEntity<StreamingResponseBody> getCanvas(@PathVariable String identifier,
            @PathVariable String version, @PathVariable String filename, HttpServletRequest req,
        ...
        if (imgSeqNum == null)
            throw new BDRCAPIException(500, AppConstants.GENERIC_LDS_ERROR,
                    "Cannot find filename in the S3 image list");
        ...
        final Canvas res = ManifestService.getCanvasForIdentifier(id, vi, imgSeqNum, id.getImageGroupId(),
                imageInfoList);
        if (vi.access == AccessType.OPEN) {
            return ResponseEntity.status(HttpStatus.OK)
                    .cacheControl(CacheControl
    ...
    public ResponseEntity<Object> getImageList(@PathVariable String volumeId, HttpServletRequest request,
            HttpServletResponse resp) throws BDRCAPIException {
        resp.setContentType("application/json;charset=UTF-8");
        ImageGroupInfo vi;
        try {
        ...
        return ResponseEntity
                .status(HttpStatus.OK).cacheControl(CacheControl
                        .maxAge(Long.parseLong(AuthProps.getProperty("max-age")), TimeUnit.SECONDS).cachePublic())
    ...
    public ResponseEntity<StreamingResponseBody> getImageInfoFile(@PathVariable String resourceQname,
            HttpServletRequest request, HttpServletResponse resp) throws BDRCAPIException {
        ...
        return ResponseEntity
                .status(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON_UTF8).cacheControl(CacheControl
                        .maxAge(Long.parseLong(AuthProps.getProperty("max-age")), TimeUnit.SECONDS).cachePublic())
    ...
    public ResponseEntity<String> writeImageInfoFile(@PathVariable String resourceQname, @RequestBody String json,
            HttpServletRequest request, HttpServletResponse resp) throws BDRCAPIException {
        BVM bvm;
        if (!resourceQname.startsWith("bdr:I"))
            throw new BDRCAPIException(404, AppConstants.GENERIC_APP_ERROR_CODE, "no resource " + resourceQname);
        ...
        if (!bvm.imageGroupQname.equals(resourceQname))
            throw new BDRCAPIException(422, GENERIC_APP_ERROR_CODE,
                    "invalid bvm: for-volume doesn't match the resource url " + resourceQname);
        ...
        if (f.exists()) {
            ...
            if (oldBvm != null) {
                if (oldBvm.rev != null && !oldBvm.rev.equals(bvm.rev)) {
                    ...
                    // throw new BDRCAPIException(409,
                    // AppConstants.GENERIC_APP_ERROR_CODE,
                    // "document update conflict, please update to the latest
                    // version.");
    ...
    public ResponseEntity<String> readAuthModel() {
        ...
        return ResponseEntity.ok("Updated auth Model was read into IIIF presentation serv");
    }
