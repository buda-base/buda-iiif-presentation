...
public abstract class Resource<T> implements Choice<T> {
    ...
    /*
     * @JsonProperty("viewingHint") private List<ViewingHint> viewingHints;
     */
    @JsonProperty("viewingHint")
    private String viewingHints;
    ...
    /*
     * public List<ViewingHint> getViewingHints() { return viewingHints; }
     */
     ...
     * @throws IllegalArgumentException if the resources not not support one of the
     *                                  viewing hints.
     */
    /*
     * public void setViewingHints(List<ViewingHint> viewingHints) throws
     * IllegalArgumentException { for (ViewingHint hint : viewingHints) { boolean
     * supportsHint = (hint.getType() == ViewingHint.Type.OTHER ||
     * this.getSupportedViewingHintTypes().contains(hint.getType())); if
     * (!supportsHint) { throw new IllegalArgumentException(String.format(
     * "Resources of type '%s' do not support the '%s' viewing hint.",
     * this.getType(), hint.toString())); } } this.viewingHints = viewingHints; }
     */
    public void setViewingHints(String viewingHints) throws IllegalArgumentException {
     ...
     */
    /*
     * public Resource addViewingHint(ViewingHint first, ViewingHint... rest) throws
     * IllegalArgumentException { List<ViewingHint> hints = this.viewingHints; if
     * (hints == null) { hints = new ArrayList<>(); }
     * hints.addAll(Lists.asList(first, rest)); this.setViewingHints(hints); return
     * this; }
     */
     ...
     *
     * @throws IllegalArgumentException if at least one rendering does not have both
     *                                  a profile and a format.
     */
    ...
    /**
     * Add one or more renderings. All renderings must have both a profile and a
     * format.
     *
     * @throws IllegalArgumentException if at least one rendering does not have both
     *                                  a profile and a format.
